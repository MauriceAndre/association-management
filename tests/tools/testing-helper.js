        ...
        keys.forEach((key) => {
            ...
            switch (obj.type) {
                ...
                case 'email':
                    ...
                    break;
                case undefined:
                    winston.info('Type is undefined.', obj);
                    return;
                default:
    ...
    test: {
        string: {
            ...
            run: function (schema, key, obj, status, exec) {
                ...
                this.min(schema, key, `less than ${obj.min} chars`, status, (body) => {
                    body[key] = new Array(obj.min).join('a');
                }, exec);
                this.max(schema, key, `more than ${obj.max} chars`, status, (body) => {
                    body[key] = new Array(obj.max + 2).join('a');
                }, exec);
                this.required(schema, key, 'missing', status, (body) => {
                    delete body[key];
                }, exec);
            }
        },
        number: {
            ...
            run: function (schema, key, obj, status, exec) {
                ...
                this.min(schema, key, `less than ${obj.min}`, status, (body) => {
                    body[key] = obj.min - 1;
                }, exec);
                this.max(schema, key, `more than ${obj.max}`, status, (body) => {
                    body[key] = obj.max + 1;
                }, exec);
                this.required(schema, key, 'missing', status, (body) => {
                    delete body[key];
